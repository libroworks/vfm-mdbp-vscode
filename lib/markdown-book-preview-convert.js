...
class MarkdownBookPreviewConvert {
  ...
  static convertMarkdown(mdpath, homepath) {
    ...
    try {
      const replisttext = fs.readFileSync(
        path.join(homepath, '_postReplaceList.json'),
        'utf-8'
      );
      ...
      vscode.window.showErrorMessage('Replist Open Error' + err.message);
    ...
    // 行番号処理（VFMの仕様に合わせて要変更）
    $('section.codenumber').each(function (i, elem) {
      ...
      $(elem)
        ...
        .each(function (i, codeelem) {
          ...
          if (content) {
            ...
            for (const s of arr) {
              output +=
                '<span class="codenum-elem">' +
                ('000' + start_number).slice(-3) +
                '</span>' +
                s +
                '\n';
              start_number++;
      ...
      for (let i = 0; i < mnplist.length; i++) {
        ...
        switch (mnplist[i].method) {
          ...
          case 'wrapAll':
            ...
            $(mnplist[i].selector + '+' + mnplist[i].paramator[0]).each(
              function () {
                const newelem = $(this)
                  .before($(mnplist[i].paramator[1]))
                  .prev();
                console.log(
                  mnplist[i].selector + '+' + mnplist[i].paramator[0]
                );
  ...
  static svgimg(mdtext, workdir) {
    ...
    // 置換実行
    const mdsvgtext = mdtext.replace(
      /!\[[^\]]*\]\(([^\)]+)\)/g,
      function (str, $1) {
        ...
        if (params.length > 1 && params[1].length > 0)
          trimW = parseFloat(params[1]);
        if (params.length > 2 && params[2].length > 0)
          trimH = parseFloat(params[2]);
        if (params.length > 3 && params[3].length > 0)
          shiftX = parseFloat(params[3]);
        if (params.length > 4 && params[4].length > 0)
          shiftY = parseFloat(params[4]);
  ...
  static exportInDesignXML(htmlfile) {
    ...
    //書き出しファイル名
    let xmlfilepath =
      path.join(workfolder, path.basename(htmlfile, '.html')) + '.xml';
